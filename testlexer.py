...
#        ("F6",         r'{F6:(\d)}'),
        ("F6",         r'!F6!'),
        ("WHITESPACE", r'[ \t\n\f]+'), # iswhitespace()
        ("COMMAND",    r'{[\S]+}'),     # {red}, {brightyellow}, etc
        ("WORD",       r'[\S]+'),
...
#    print("pos=%d" % (pos))
    if token.type == "F6":
        ...
        if pos+len(token.value) > terminalwidth:
            result += "\n" # @@
            pos = len(token.value)
            result += token.value
        else:
